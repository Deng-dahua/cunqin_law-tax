...
def gen_html(tpl, art):
    ...
    # 2. <title> tag
    h = h.replace(
        '<title>金税四期全面解读：企业如何从容应对以数治税新时代 - 存勤法税服务（广州）有限公司</title>',
        f'<title>{title} - 存勤法税服务（广州）有限公司</title>'
    )

    # 3. meta description (two occurrences)
    old_desc_1 = 'content="金税四期全面解读与应对策略：从发票电子化改革趋势、大数据风险画像机制、全税种联动监控原理三个维度，深入剖析金税四期对企业财税管理的实质性影响，提供发票管理升级路径、业务合规整改方案、风险自查机制建设等实操应对指南。面对金税四期，企业需要的不是恐慌，而是专业的合规升级路径。"'
    new_desc = f'content="{desc}"'
    h = h.replace(old_desc_1, new_desc)

    # 4. OG tags
    h = h.replace('jinshui-siqi-yingdui.html', f'{slug}.html')
    h = h.replace('金税四期全面解读：企业如何从容应对&quot;以数治税&quot;新时代', title)
    # og:description (second occurrence)
    old_desc_og = 'content="深度解读金税四期政策要点与实施背景，剖析发票电子化、大数据风险监控对企业财税管理的影响，提供切实可行的税务合规应对策略"'
    h = h.replace(old_desc_og, f'content="{desc}"')

    # 5. hero cat-tag
    h = h.replace('政策解读', cat)
    h = h.replace('原创', '原创')
    # hero h1
    h = h.replace('金税四期全面解读：企业如何从容应对"以数治税"新时代', title)
    # hero date
    ...
    # 7. Schema Article
    h = h.replace('"headline": "金税四期全面解读：企业如何从容应对\\"以数治税\\"新时代"', f'"headline": "{title}"')
    h = h.replace('"datePublished": "2026-04-15"', f'"datePublished": "{date}"')
    h = h.replace('"dateModified": "2026-05-21"', f'"dateModified": "2026-05-24"')
    # Schema BreadcrumbList third item name
    h = h.replace('"name": "金税四期全面解读：企业如何从容应对\\"以数治税\\"新时代"', f'"name": "{title}"')
    # Schema mainEntityOfPage @id
    h = h.replace('/articles/jinshui-siqi-yingdui.html', f'/articles/{slug}.html')

    # 8. Replace article body
    ...
    # 10. FAQPage Schema (replace the entire FAQPage block)
    faq_schema = json.dumps(faqs, ensure_ascii=False, indent=6)
    # Find FAQPage block start and end
    faq_start = h.find('"@type": "FAQPage"')
    if faq_start != -1:
        # Find the opening [ and closing ] for mainEntity
        # Go back to find the start of FAQPage object
        faq_obj_start = h.rfind('{', 0, faq_start)
        ...
        # Replace from faq_obj_start to faq_obj_end (inclusive)
        h = h[:faq_obj_start] + '{\n    "@context": "https://schema.org",\n    "@type": "FAQPage",' + new_faq_block + h[faq_obj_end+1:]
    else:
        print(f"  ✗ 找不到FAQPage Schema: {slug}")
