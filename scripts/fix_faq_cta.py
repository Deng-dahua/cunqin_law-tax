#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix FAQPage Schema and CTA text for the 3 new articles."""
import re, os

BASE = r'C:\Users\26726\WorkBuddy\2026-05-20-21-20-24\source\articles'

# ============================================================
# FAQPage replacements (old template → new per article)
# ============================================================
FAQ_OLD = r'''"@type": "FAQPage",
    "mainEntity": \[
      \{"@type": "Question",
        "name": "什么是企业税务风险？",
        "acceptedAnswer": \{"@type": "Answer",
          "text": "税务风险指企业因涉税行为不符合税法规定而可能遭受的法律制裁、财务损失和声誉损害，包括政策理解偏差、核算错误、发票违规等。"
        \}
      \},
      \{"@type": "Question",
        "name": "如何建立税务风险管控体系？",
        "acceptedAnswer": \{"@type": "Answer",
          "text": "需建立税务风险管理制度，设置专门岗位或人员，定期开展税务健康检查，运用数字化工具实时监控。"
        \}
      \},
      \{"@type": "Question",
        "name": "税务风险管控的具体步骤是什么？",
        "acceptedAnswer": \{"@type": "Answer",
          "text": "分为四步闭环：风险识别（发现风险点）→风险评估（量化风险等级）→风险控制（制定应对方案并执行）→风险预警（建立持续监控机制），循环迭代，持续优化。"
        \}
      \},
      \{"@type": "Question",
        "name": "如何进行税务健康检查？",
        "acceptedAnswer": \{"@type": "Answer",
          "text": "参照税务机关稽查清单，逐项核查收入确认、成本费用、发票管理、优惠享受、关联交易等核心领域，可聘请外部专业人士进行独立检查，出具健康检查报告。"
        \}
      \},
      \{"@type": "Question",
        "name": "税务风险管控有哪些常见误区？",
        "acceptedAnswer": \{"@type": "Answer",
          "text": "认为\\"合理避税\\"等同于偷税、等问题出现再处理、过度依赖中介机构而不建内生能力、忽视业务前端而仅关注财务后端等。"
        \}
      \}
    \]'''

FAQ_MAP = {
    '税务稽查应对实战手册(source).html': '''"@type": "FAQPage",
    "mainEntity": [
      {"@type": "Question",
        "name": "收到税务稽查通知书后该怎么办？",
        "acceptedAnswer": {"@type": "Answer",
      ...
      {"@type": "Question",
        "name": "被税务稽查时要不要请律师？",
        "acceptedAnswer": {"@type": "Answer",
          "text": "建议税务顾问和律师组合使用。税务顾问负责专业领域的实体问题（税务政策适用、计算方法、业务定性），律师负责程序问题（证据合法性审查、法律文书应对、复议和诉讼代理）。当稽查涉及大额补税和罚款、或可能涉及移送公安时，律师的参与尤为重要。税务顾问在陈述申辩阶段更具优势，律师在复议和诉讼阶段更具优势。"
        }
      }
    ]''',

    '企业税务健康体检30项(source).html': '''"@type": "FAQPage",
    "mainEntity": [
      {"@type": "Question",
        "name": "企业税务健康检查有什么用？",
        "acceptedAnswer": {"@type": "Answer",
    ...
    '私户收款被查补救指南(source).html': '''"@type": "FAQPage",
    "mainEntity": [
      {"@type": "Question",
        "name": "个人卡收款被查到怎么办？",
        "acceptedAnswer": {"@type": "Answer",
...
# Execute
# ============================================================
for filename in FAQ_MAP:
    filepath = os.path.join(BASE, filename)
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # Replace FAQPage
    new_faq = FAQ_MAP[filename]
    new_content = re.sub(FAQ_OLD, new_faq, content, flags=re.DOTALL)
    if new_content == content:
        print(f'  [WARN] FAQPage NOT replaced in {filename}')
    else:
        print(f'  [OK] FAQPage replaced in {filename}')

    # Replace CTA heading
    cta_new = CTA_MAP[filename]
    new_content = new_content.replace(CTA_OLD, cta_new)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)

print('\nDone! FAQPage + CTA updated for all 3 articles.')

# Verify FAQPage count
for filename in FAQ_MAP:
    filepath = os.path.join(BASE, filename)
    with open(filepath, 'r', encoding='utf-8') as f:
        c = f.read()
    count = c.count('"@type": "FAQPage"')
    print(f'  {filename}: FAQPage count = {count}')
