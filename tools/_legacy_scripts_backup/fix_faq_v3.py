#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fix FAQPage Schema - correct version using string-based replacement."""
import os

BASE = r'C:\Users\26726\WorkBuddy\2026-05-20-21-20-24\source\articles'

# New FAQPage blocks (5 questions each)
FAQ_NEW = {
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "非常需要。越是中小企业，税务风险越高。原因有三：中小企业通常缺乏专门的税务管理团队，问题更隐蔽；金税四期的数据比对不因企业规模而区别对待；中小企业抗风险能力更弱，一次大额补税就可能影响正常经营。建议至少每半年做一次自检，每年请专业机构做一次深度体检。"
        }
      },
      {
        "@type": "Question",
...
# Old FAQPage block markers (the exact start and end lines from the template)
# Start: '  {\n    "@context": "https://schema.org",\n    "@type": "FAQPage",'
# End: the last '  }' before '\n]\n  </script>'
# We know the FAQPage block is between mark1 and mark2

MARK_FAQ_START_FLAG = '"@type": "FAQPage",'
MARK_JSON_END = '\n]\n  </script>'

def find_faq_block(content):
    """Find start and end positions of the FAQPage JSON block."""
    idx = content.find(MARK_FAQ_START_FLAG)
    if idx == -1:
        return None, None
    
    # Find the opening "{" of the FAQPage object (2 lines before @type)
    start_search = content.rfind('\n  {', 0, idx)
    if start_search == -1:
        return None, None
    
    # Find the closing of this specific object
    # The FAQPage object is followed by "\n]\n  </script>"
    # Find the "}" that closes FAQPage, which is just before "\n]\n  </script>"
    end_marker = content.find(MARK_JSON_END, idx)
    if end_marker == -1:
        return None, None
    
    # Walk backwards from end_marker to find the closing "  }" of FAQPage
    close_pos = content.rfind('\n  }', start_search, end_marker)
    if close_pos == -1:
        return None, None
    
    return start_search, close_pos + 4  # +4 for "\n  }"
...
for filename, new_faq in FAQ_NEW.items():
    ...
    start, end = find_faq_block(content)
    if start is None or end is None:
        print(f'[FAIL] Could not find FAQPage block in {filename}')
        continue
    
    print(f'Replacing FAQPage ({start}-{end}) in {filename}')
    new_content = content[:start] + new_faq + '\n' + content[end:]
    
    # Also replace CTA
    new_content = new_content.replace(CTA_OLD, CTA_MAP[filename])
...
for filename in FAQ_NEW:
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        c = f.read()
    total = c.count('"@type": "Question"')
    faq_idx = c.find('"@type": "FAQPage"')
    post_faq = c[faq_idx:faq_idx+8000] if faq_idx >= 0 else ''
    faq_q = post_faq.count('"@type": "Question"')
    print(f'  {filename}: totalQ={total}, inFAQ={faq_q}')
