...
读取 tools/article_bodies/ 下的txt文件作为正文，
基于 金税四期全面解读(source).html 模板，替换关键字段后输出到 source/articles/
"""
...
# FAQ模板（每篇文章5个问答，这里用通用版，实际应每篇不同）
# 为节省篇幅，FAQ在生成时根据标题动态生成
...
def make_faq(title):
    """为每篇文章生成5个相关问题（通用模板，实际应根据文章内容定制）"""
    return [
        {"q": f"{title}中最重要的税务要点是什么？",
         "a": f"本文系统讲解了{title}的核心内容，建议重点阅读前言、核心规则解析和筹划策略部分，掌握合法合规的操作要点。"},
        {"q": f"企业在处理{title}相关事务时，最常见的错误有哪些？",
         "a": "最常见的错误包括：对政策理解不准确、备查资料不完整、未按期申报或扣缴、混淆不同税目的适用条件等。建议企业在处理前咨询专业税务顾问。"},
        {"q": f"{title}相关的税务优惠有哪些？",
         "a": "具体优惠内容因文章主题而异，建议仔细阅读本文「税收优惠」或「筹划策略」章节，并用足用好各项扣除、抵免和减免政策。"},
        {"q": f"金税四期对{title}相关税务处理有什么影响？",
         "a": "金税四期通过大数据比对、AI智能分析、跨部门数据互通，实现了对企业税务行为的全方位监控。建议企业建立完善的合规管理体系，确保「四流合一」。"},
        {"q": f"如果我对{title}的税务处理仍有疑问，应该怎么解决？",
         "a": "建议联系存勤法税。我们是专注业财税法融合的专业顾问机构，可提供一对一的定制化税务解决方案。咨询热线：13556116691（微信同号）。"},
    ]
...
def gen_html(tpl, art):
    ...
    h = re.sub(r'<title>.*?</title>',
        f'<title>{title} - 存勤法税服务（广州）有限公司</title>', h, flags=re.DOTALL)

    # 3. meta description (两次出现)
    h = re.sub(r'content="金税四期全面解读.*?>"',
        f'content="{desc}">', h, flags=re.DOTALL)

    # 4. OG/Twitter URLs & titles
    h = h.replace('jinshui-siqi-yingdui.html', f'{slug}.html')
    h = h.replace('金税四期全面解读：企业如何从容应对&quot;以数治税&quot;新时代', title)
    h = h.replace('金税四期全面解读与应对策略：从发票电子化改革趋势', title[:60] if len(title)>60 else title)

    # 5. Schema Article headline + date
    h = re.sub(r'"headline":\s*".*?"',
        f'"headline": "{title}"', h)
    h = re.sub(r'"datePublished":\s*"\d{4}-\d{2}-\d{2}"',
        f'"datePublished": "{date}"', h)
    h = re.sub(r'"dateModified":\s*"\d{4}-\d{2}-\d{2}"',
        f'"dateModified": "2026-05-24"', h)

    # 6. Hero: cat-tag + h1 + date + view slug
    h = h.replace('政策解读', cat)   # cat-tag
    h = h.replace('金税四期全面解读：企业如何从容应对"以数治税"新时代', title)
    h = h.replace('2026-04-15', date)
    h = re.sub(r'data-slug="jinshui-siqi-yingdui"', f'data-slug="{slug}"', h)
    h = re.sub(r'id="view-jinshui-siqi-yingdui"', f'id="view-{slug}"', h)
    h = re.sub(r'view-jinshui-siqi-yingdui', f'view-{slug}', h)

    # 7. 面包屑
    h = h.replace('金税四期全面解读：企业如何从容应对"以数治税"新时代</span>', f'{title}</span>')

    # 8. 替换正文（找到 <article class="article-body"> ... </article>）
    # 找正文起止位置
    ...
    h = h.replace('jinshui-siqi-yingdui', slug)

    # 10. FAQPage Schema 替换
    faq_schema = json.dumps(faqs, ensure_ascii=False, indent=6)
    # 找到 FAQPage 的 mainEntity 并替换
    h = re.sub(r'"@type":\s*"FAQPage".*?"mainEntity":\s*\[.*?\]',
